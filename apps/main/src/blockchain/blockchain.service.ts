...
export class BlockchainService {
  ...
  constructor(
    ...
    const nodeUrl = `https://sepolia.infura.io/v3/${this.config.get(
  ...
  async callStoreFunction(request: ContractRequest) {
    try {
      ...
      const pvKey = this.config.get('BLOCKCHAIN_ACCOUNT_PRIVATE_KEY');
      ...
      const contract = await new this.web3.eth.Contract(
        ...
        this.config.get('BLOCKCHAIN_CONTRACT_ADDRESS')
      );
      ...
      //   to: this.config.get('BLOCKCHAIN_CONTRACT_ADDRESS')
